...
GaussianMixtureRegression::GaussianMixtureRegression() = default;
...
int GaussianMixtureRegression::print_parameters() const
{
...
int GaussianMixtureRegression::run_inference(const std::vector<Eigen::VectorXd> &_x,
                                             std::vector<Eigen::VectorXd> &_mean,
                                             std::vector<Eigen::MatrixXd> &_sigma,
                                             int number_of_input_variables)
{
    const int number_of_output_variables = number_of_variables - number_of_input_variables;
    const int input_start_index = 0;
    const int output_start_index = number_of_input_variables;
    ...
    for (const auto &x : _x)
    {
        ...
        {
            std::cout << "Input x: "
                      << x.transpose().format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", "\n"))
                      << std::endl;
        }
        ...
        for (int i = 0; i < number_of_states; ++i)
        {
            ...
            Eigen::MatrixXd s = sigma[i].block(input_start_index, input_start_index,
                                               number_of_input_variables, number_of_input_variables);
        ...
        for (int i = 0; i < number_of_states; i++)
        {
            ...
            auto sigma_temp =
                sigma[i].block(output_start_index, input_start_index, number_of_output_variables,
                               number_of_input_variables) *
                sigma[i]
                    .block(input_start_index, input_start_index, number_of_input_variables,
                           number_of_input_variables)
                    .inverse();
        ...
        Eigen::MatrixXd sigma_pred =
            Eigen::MatrixXd::Zero(number_of_output_variables, number_of_output_variables);
        for (int i = 0; i < number_of_states; i++)
        {
            auto s =
                sigma[i].block(output_start_index, output_start_index, number_of_output_variables,
                ...
                sigma[i].block(output_start_index, input_start_index, number_of_output_variables,
                    ...
                    sigma[i]
                        .block(input_start_index, input_start_index, number_of_input_variables,
                               number_of_input_variables)
                        .inverse() *
                    sigma[i].block(input_start_index, output_start_index, number_of_input_variables,
                                   number_of_output_variables);
...
double GaussianMixtureRegression::gaussPDF(const Eigen::VectorXd &x, const Eigen::VectorXd &mu,
                                           const Eigen::MatrixXd &sigma, int nbVar) const
{
    Eigen::VectorXd diff = x - mu;
    Eigen::VectorXd e = -diff.transpose() * sigma.inverse() * diff / 2;
    return std::exp(e(0)) / std::sqrt(std::pow(M_PI * 2, nbVar) * sigma.determinant());
}
